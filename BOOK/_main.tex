...
\caption{\label{essay1qtijs}Simple essay exercise rendered by qtijs}
\end{figure*}
...
If you want to have more fine-grained control, consider the RMD template \texttt{directedpair\ (complex)}, which uses more YAML attributes.
...
Note that setting these two attributes in the Rmd file directly is mainly useful if you work on a single task. For collections of tasks (tests) it is better to set these attributes when creating the test instead of in the Rmd-file.
