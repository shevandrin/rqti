...
Canvas is transitioning to a new quizzes interface, so maybe this one
will work better.

The following functionalities encounter problems when used with Canvas:

- Sometimes items are imported without error message but are not
  displayed.
...
ILIAS does not explicitly state which version of QTI it supports.
However, it seems to rely on QTI 1.2.

As a consequence, files generated with modern tools targeting QTI 2.1
(such as *rqti*) are not compatible with ILIAS. Given that QTI 1.x is
deprecated and no longer aligns with current specifications, we do not
plan to support it going forward.

## Blackboard

Blackboard supports QTI 2.1, but with a significant limitation:

> “Questions using the optional QTI response processing are skipped and
> not imported.”
> (<https://help.anthology.com/blackboard/instructor/en/assessments/questions/reuse-questions/qti-packages.html>)

In practice, this restriction is critical. QTI response processing
defines how answers are evaluated and scored. If such questions are
omitted during import, essential parts of the assessment logic are lost.

From our perspective, importing questions without their associated
response processing undermines the integrity of the assessment.
Consequently, this limitation substantially reduces the usefulness of
QTI 2.1 imports in Blackboard.
...
`/api/v1/accounts/:account_id/content_migrations`. Also see:
<https://community.instructure.com/en/kb/articles/658474-classic-quizzes-vs-new-quizzes-feature-comparison>
and
<https://developerdocs.instructure.com/services/new-quizzes/public/api-endpoints/new_quizzes>
as well as
