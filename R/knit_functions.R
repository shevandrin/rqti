...
#'
#' Choosing a specific Rmd file the function generates the qti.xml for it
#' through the functions of the package exams, after the XML is copied into the
#' QTI Engine which transforms the qti.xml into HTML format. Finally, the HTML
...
knit_qti_html <- function(input, ...) {
  ...
  if (RCurl::url.exists(url_engine) == TRUE) {
    ...
    print(paste("QTI Engine not available in ",
                url_engine,
                ". Start the server with function start_server()"))
  }
}

#' Start QTI Engine in a server
#'
#' The function starts a server in the folder that contains the js QTI Engine
#' that performs the conversion of qti.xml into HTML.
#'
#' @param qti_engine_f filepath to the js QTI Engine. Can be empty to indicate
#'   that the server can be initiated in the package folder.
#'
#' @examples
#' \dontrun{
