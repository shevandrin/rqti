...
Class `DirectedPair` is responsible for creating assessment tasks
according to the QTI 2.1 standard, where a candidate has to make binary
associations between answer options.
...
- `points`:
  ...
  - For tasks of the DirectedPair, the default is calculated as 0.5
    points per pair.
...
- `rows`:
  ...
  or the first elements in couples in DirectedPair.
...
- `cols`:
  ...
  table or the second elements in couples in DirectedPair.
