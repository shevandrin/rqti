...
This is just a typical essay exercise. A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`.  The rqti templates start with `RQTI:`. Here we look at the templates `RQTI: essay (simple)` and `RQTI: essay (complex)`.
