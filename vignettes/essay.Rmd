...
`r if(knitr::is_latex_output()) "# Essay Exercises"`

This is a standard essay exercise with an open field for students to write a comprehensive response.
...
Knitting via the Knit-Button to qtijs, this exercise renders asd`r ltx(" shown in Figure \\ref{essay1qtijs}.", ":")`

![\label{essay1qtijs}Simple essay erxercise rendered by qtijs](images/xml/essay-simple.jpg){width=100%}

Alternatively, change the knit parameter to `knit: render_opal` (see [Working with the OPAL API](api_opal.html)) to upload to Opal directly, producing`r ltx(" the output in Figure \\ref{essay1opal}.", ":")`

![\label{essay1opal}Simple essay exercise rendered by OPAL](images/essay-simple.jpg){width=100%}
...
![\label{essay2opal}More complex essay exercise rendered in OPAL](images/essay-complex.jpg){width=100%}
