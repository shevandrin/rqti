...
In this type of exercise, the candidate has to select an element form a dropdown-list. Note that our package implements dropdowns as gaps because this is essential what dropwdowns are. Several dropdowns can be combined in a single exercise, but a combination with numeric and text gaps is not possible.A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`. The `rqti` templates start with `RQTI:`. Here we look at the templates `RQTI: dropdown (simple)` and `RQTI: dropdown (complex)`.
...
If you want to have more fine-grained control, consider the RMD template `RQTI: dropdown (complex)`, which uses more yaml attributes.
