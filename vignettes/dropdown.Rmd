...
In this type of task, the candidate has to select an element from a dropdown list. Note that our package implements dropdowns as *entry*-objects because this is essentially what dropdowns are. Several dropdowns can be combined in a single task, but a combination with numeric and text entries (gaps) is not possible, mainly due to limitations of learning management systems. 
...
```{r comment='', echo = F}
cat(readLines(system.file("rmarkdown/templates/dropdown-simple/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
By default, 1 point can be earned for each dropdown. Adjust `points` to your needs. The total number of points for completing a task is defined as the sum of points of all dropdowns.
...
```{r comment='', echo = F}
cat(readLines(system.file("rmarkdown/templates/dropdown-complex/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
Elements of the dropdown. If you use a named vector, the names will be used as identifiers. This is useful for later data analysis and is generally advised.
