...
A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`.  The `rqti` templates start with `RQTI:`. Here we look at the templates `RQTI: multiplechoice (simple)` and `RQTI: multiplechoice (complex)`.
...
```{r comment='', echo = F}
cat(readLines(fs::path_package("exercises", "mpc1.Rmd", package = "rqti")), sep = '\n')
```
...
If you want to have more fine-grained control, consider the available attributes for the yaml section in the RMD template `RQTI:multiple-choice (complex)`. In addition you can also set feedback for correct and incorrect responses.
