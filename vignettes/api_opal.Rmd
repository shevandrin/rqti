...
The `qti` package can upload content directly to the learning management system OPAL via the function `upload2opal`, which uses the OPAL API.

1. **Uploading a Zip Archive:**
If you already have a zip archive containing a QTI compliant test, it will be uploaded directly to OPAL.

2. **Uploading an Rmd File:**
If you have an Rmd exercise file, `upload2opal` wraps it into a zip archive and then uploads it to OPAL.

3. **Uploading an XML File:**
If you have a QTI compliant exercise file as XML, `upload2opal` wraps it into a zip archive and then uploads it to OPAL.

## Prerequisites

To use the OPAL API, the OPAL user must have sufficient permissions. Your username should allow you to log in using a password. Working with the API system via shibboleth authorization is not possible.

For authentication to OPAL `qti` uses two global environment variable: `API_USER` and `API_PASSWORD`. To set them, you need to call `Sys.setenv(API_USER ='xxxxxxxxxxxxxxx')` and `Sys.setenv(API_PASSWORD ='xxxxxxxxxxxxxxx')` or you can set these variables in .Renviron:
...
For TU Chemnitz [see the instructions here](https://www.tu-chemnitz.de/urz/network/access/vpn.html.en#client)
...
Your API_USER and/or API_PASSWORD are wrong or your credentials are not sufficient to work with API.
