...
## Uploading exercises and tests to OPAL
...
Note that the access rights of the resource are set to public by default. This might appear unusual, but for creating exercises incrementally this is the best option. If you use any other access rights, you need to login to OPAL via your browser to see the web page, which will log you out via the API. Currently, there is not better solution for this procedure. Take this into account if you upload sensitive content! For instance, exercises that will be used in an upcoming exam. You can set the access rights via
...
There are several opal-specific yaml attributes, which are explicitly explained in the articles on the exercise types. Two of them apply on the test level:
...
Note that setting these two attributes in the Rmd file directly is mainly useful if you work on a single exercise. For collections of exercises (tests) it is better to set these attributes when creating the test intead of in the Rmd-file.
