...
There are several OPAL-specific YAML attributes, which are explicitly explained in the articles on the task types. Two of them apply at the test level:
...
Note that setting these two attributes directly in the Rmd file is mainly useful if you work on a single task. For collections of tasks (tests), it is better to set these attributes when creating the test instead of in the Rmd file.
