...
`r if(knitr::is_latex_output()) "# Single-Choice Exercises"`

```{r setup sc, echo = F, message=F}
library(rqti)
```

Please note that this is the first article in this manual, and as such, it may be more detailed than those that follow. Our goal is to emphasize key points that might not be as thoroughly covered in later articles. For this reason, we recommend reading through this article carefully before diving into the exercise types that most interest you.
...
You can now interact with the exercise by selecting an option and then clicking `Submit` in the top right corner. By default, after submitting, the feedback and reached points are displayed`r ltx(" as shown in Figure \\ref{sc1qtijs}.", ":")`

![\label{sc1qtijs}Preview of single choice exercise with feedback rendered by QTIJS](images/xml/scfb.jpg){width=100%}
...
Many learning management systems can directly import a QTI-XML-file, so all you need to do is upload the generated file. Compositions of exercises are covered in the article \ref{sections-and-tests} [Sections and Tests](section.html).

If you happen to use OPAL/ONYX, you can also upload your exercises directly by modifying the knit parameter:
...
![\label{sc1opal}Single choice exercise renderd in OPAL](images/singlechoice-simple.jpg){width=100%}
...
![\label{sc2opal}More complex single choice exercise rendered in OPAL](images/singlechoice-complex.jpg){width=100%}
...
Thus, there is no need to go beyond using the general feedback.
...
From a psychometric standpoint, single-choice exercises are often the least effective option for assessing ability. This is primarily because guessing cannot be entirely ruled out, leading to weaker psychometric properties compared to similar content presented in gap exercises.

For example, the `exams` package, an alternative R package for test creation, frequently converts numeric gaps into single-choice exercises. However, we believe this approach is justified only when the learning management system either does not support gap exercises, provides inadequate support, or the instructor requires printed exams with automated grading. In all other cases, using numeric or string gaps is generally more effective.

In some cases, single-choice exercises may be unavoidable. For instance, when determining whether a statistical test is significant, there are only two possible answers. In such situations, we suggest asking multiple related questions rather than relying on a single one. Additionally, assigning fewer points to single-choice exercises can help mitigate the effects of guessing.

If you have multiple single-choice exercises with identical answer options, consider using a match table instead (see article \ref{table-exercises} [Table Exercises](table.html)).

In conclusion, it is advisable to avoid single-choice exercises whenever possible. Specifically, refrain from converting numeric gap exercises into single-choice format unless absolutely necessary. If single-choice exercises are unavoidable, consider asking multiple related questions or using a match table. Additionally, reducing the weight of single-choice exercises in the overall grading can help create a more balanced and fair assessment.
