...
The simplest exercise type in the rqti package is single choice.A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`. The `rqti` templates start with `RQTI`. Here we look at the templates `RQTI: singlechoice (simple)` and `RQTI: singlechoice (complex)`.
...
If you want to have more fine-grained control, consider the RMD template `RQTI: singlechoice-complex`, which uses many yaml attributes. In addition you can also set feedback for correct and incorrect responses.
