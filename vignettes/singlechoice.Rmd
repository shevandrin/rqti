...
The simplest exercise type in the qti package is single choice. A template is available in RStudio when you click on `New file -> R Markdown -> From Template`. The qti templates start with `QTI:`. Here we look at the templates `QTI: singlechoice (simple)` and `QTI: singlechoice (complex)`.
...
Note that in this example, a feedback section was also provided. This is optional, but usually it is a good idea to give some explanation for students (here not really since it is trivial).

Further note that the `knit` parameter is set to a custom qti function, which will handle the preview. This is not necessarily required, but simplifies the workflow substantially. If you leave it out, the default preview is just an html file. If you add our knit function, you will get a more realistic preview.
...
<!-- # try out pageres instead? -->
<!-- system(paste0("wkhtmltoimage ", "http://127.0.0.1:6052 ", "sc1.png")) -->
<!-- ``` -->

To see what the final result might look like in a learning management system you can just click the `Knit` button in RStudio. This will produce a qti xml file and render it in the viewer pane via QTIJS (you can also just fire up the displayed server url in the browser). The same is achieved when you run `render_qti` on the Rmd-file or first `rmd2qti` (producing the qti xml) and then `render_xml` to render the file. In every case the output looks like this:

![Preview of single choice exercise rendered by QTIJS](screenshot_sc1.png){width="400"}

The corresponding xml file is created in the same folder as the Rmd file if you click the Knit-Button or in the current directory if you execute the functions from the console. Usually, the Knit button will be sufficient for most use cases, but if you need more control the functions `rmd2qti, render_xml, render_zip` are handy. The functions give a message about the path of the created file.

Most learning management systems can directly import an xml-file, so all you need to do is upload the generated file. Compositions of exercises are covered in the article: ....TODO

If you happen to use OPAL/ONYX, you can also upload your exercises via:

```{r eval=FALSE}
rmd2qti("sc1.Rmd") # generates xml
url <- upload2opal("sc1.xml")
```

The last function will return a url, which you can fire up in a browser:

```{r eval=FALSE}
browseURL(url)
```

By default the rights of the uploaded material are set to public, so no login is required to view the material. Otherwise you have to login into OPAL, which will log you out in the API.

The function `upload2opal` will aks you for credentials first. Depending on the OPAL setup you might need to use a VPN. Furthermore only password-based authentication accounts work (Shibboleth does not work). You probably need to consult your IT before using `upload2opal`. More information on this can be found in the article: OPAL API. Specifically, we describe how to setup the API at TU Chemnitz.
...
url <- upload2opal("sc1b.xml") # hide
```

<!-- ![Preview of single choice exercise rendered by QTIJS](screenshot_sc1b.png){width="400"} -->
...
```{r comment='', echo = F}
cat(readLines(fs::path_package("rmarkdown/templates/singlechoice-complex/skeleton", "skeleton.Rmd", package = "qti")), sep = '\n')
```
...
mdlist(c(1.2, 2.1, 2.2, 3.0), solutions = c(2))
```
