...
cat(readLines(system.file("rmarkdown/templates/singlechoice-simple/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
```{r comment='', echo = F}
cat(readLines(system.file("rmarkdown/templates/singlechoice-complex/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
- "penalty": The respondent receives full points for a correct answer. For an incorrect answer, a penalty of $-\frac{\text{points}}{k - 1}$ is applied, where $k$ is the number of answer options. This approach is often referred to as *formula scoring*, although the term is used inconsistently in the literature. Note that in Germany, the use of penalty scoring raises legal considerations; see, for example: https://nrwe.justiz.nrw.de/ovgs/ovg_nrw/j2008/14_A_2154_08urteil20081216.html
