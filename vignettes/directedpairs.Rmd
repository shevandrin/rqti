...
In this type of exercise, the candidate is given two sets of items, which need to be matched. A template is available in RStudio when you click on `New file -> R Markdown -> From Template`. The rqti templates start with `QTI:`. Here we look at the templates `QTI: directedpair (simple)` and `QTI: directedpair (complex)`.
