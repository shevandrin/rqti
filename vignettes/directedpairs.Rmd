...
In this type of exercise, the candidate is given two sets of items, which need to be matched. A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`. The `rqti` templates start with `RQTI:`. Here we look at the templates `RQTI: directedpair (simple)` and `RQTI: directedpair (complex)`.
...
```{r comment='', echo = F}
cat(readLines(fs::path_package("rmarkdown/templates/directedpair-simple/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
If `abbr_id` is omitted, then `rqti` generates identifiers as `right_1`, `right_2`, ...`right_N` and `left_1`, `left_2`...`left_N`. This does not contain any semantics, so it can be more useful to abbreviate the identifiers, which `abbr_id` does. Depending on the length of items, the abbreviations are more or less useful, but in any case more useful than non-semantic identifiers.
