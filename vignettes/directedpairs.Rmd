---
title: "Directed Pair Exercises"
output: rmarkdown::html_vignette
date: "`r Sys.Date()`"
vignette: >
    %\VignetteIndexEntry{Directed Pair Exercises}
...
`r ltx("# Directed Pair Exercises", "")`

## Minimum version

In this type of exercise, the student is given two sets of items, which need to be matched. A template is automatically created when you initiate an rqti project through RStudio. Alternatively, it can be added by clicking on `New file -> R Markdown -> From Template`. The `rqti` templates end with `{rqti}`. Here we look at the templates `directedpair (simple)` and `directedpair (complex)`.
...
Knitting via the Knit-Button to qtijs, this exercise renders as`r ltx(" Figure \\ref{dpqtijs1}.", ":")`

![\label{dpqtijs1}Directed pair exercise rendered with qtijs](images/xml/directedpair-simple.jpg){width=100%}
...
An alternative is to provide a table exercise with the matching elements, see the article \ref{table-exercises} [Tables](table.html).
...
![\label{dpopal1}Directed pair exercise rendered in OPAL](images/directedpair-simple.jpg){width=100%}
...
In OPAL this renders as `r ltx("shown in Figure \\ref{dp2opal}.", "shown in the next image.")` The viewport is a bit too small to capture the entire exercise.

![\label{dp2opal}More complex directed pair exercise rendered in OPAL](images/directedpair-complex.jpg){width=100%}
...
How many points are given for the whole exercise. Default is $0.5\cdot n$, where $n$ is the number of pairs.
