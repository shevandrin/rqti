...
The minimum you need to provide is the `type: gap` (or the equivalent `type: cloze`) in the YAML section and some text, where at least one gap is used, in a section called  **\#question**. Furthermore, when employing helper functions from the rqti package, it is essential to ensure it is loaded first.

```{r comment='', echo = F}
cat(readLines(system.file("rmarkdown/templates/gap-simple/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
...
```{r comment='', echo = F}
cat(readLines(system.file("rmarkdown/templates/gap-complex/skeleton", "skeleton.Rmd", package = "rqti")), sep = '\n')
```
