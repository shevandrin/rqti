...
`r if(knitr::is_latex_output()) "# Gap Exercises"`

In this exercise format, candidates are required to fill in one or more gaps. We believe this method is one of the most effective ways to assess students' abilities, as it reduces the likelihood of guessing compared to multiple-choice questions. Emphasizing gap-fill exercises often leads to more reliable assessment outcomes.

Our package allows for the integration of both textual and numeric responses within a single exercise, offering instructors greater flexibility in designing assessments.
...
![\label{gap1qtijs}Simple gap exercise rendered in qtijs](images/xml/gap-simple.jpg){width=100%}

Alternatively, change the knit parameter to `knit: render_opal` (see article \ref{working-with-the-opal-api} [Working with the OPAL API](api_opal.html)) to upload to OPAL directly, producing`r ltx(" the output in Figure \\ref{gap1opal}.", ":")`

![\label{gap1opal}Simple gap exercise rendered in OPAL](images/gap-simple.jpg){width=100%}

As you can see, there are two ways to create a gap in a Rmd file:
...
![\label{gap2opal}Preview of complex gap exercise in OPAL](images/gap-complex.jpg){width=100%}
...
As you can see, YAML is ultimately used, but for most users, it is more convenient to have a dedicated R function instead.
