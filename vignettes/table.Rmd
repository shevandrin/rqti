...
In this type of exercise, the candidate must match rows and columns of a table. A template is available in RStudio when you click on `New file -> R Markdown -> From Template`. The rqti templates start with `QTI:`. Here we look at the templates `QTI: table (simple)` and `QTI: table (complex)`.
