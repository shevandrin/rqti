...
In this type of exercise, the candidate must match rows and columns of a table. A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`. The `rqti` templates start with `RQTI:`. Here we look at the templates `RQTI: table (simple)` and `RQTI: table (complex)`.
...
2. Using `abbr_id: true` in the yaml section of the Rmd file, `rqti` takes the first word of a row or column element and combines it with the abbreviation of the remaining text of the element. Example: for the element "Mean Value Theorem for Integrals" `rqti` makes the identifier "Mean_VTfI".
3. For more control, you will probably want to assign your own values as identifiers.  In this case, in the markdown table or csv file you need to put:
...
![Preview of markdown table with data for identifiers](images/screenshot_table3.png)
