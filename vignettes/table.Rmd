...
`r ltx("# Table Exercises", "")`

In this type of exercise, the candidate matches rows with columns in a table. This format is often used when multiple questions need to be displayed concisely. Table exercises are highly versatile and are ideal for creating both single-choice and multiple-choice questions.
...
![\label{tbl1qtijs}Preview of table exercise rendered by qtijs](images/xml/table-simple.jpg){width=100% border=2px}
...
![\label{tbl1opal}Preview of table exercise rendered by OPAL](images/table-simple.jpg){width=100%}
...
However, it is important to note that if your table represents a directed pair, the `rqti` package will automatically convert it into a directed pair exercise. If you prefer to keep it as a standard table, use `as_table: true` in the YAML section of your Rmd file.
...
![\label{tbl2opal}Preview of complex table exercise in the learning management system OPAL](images/table-complex.jpg){width=100%}
