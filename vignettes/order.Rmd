...
In this type of exercise, the candidate has to bring a list of items in the correct order. A template is available in RStudio when you click on `New file -> R Markdown -> From Template`. The rqti templates start with `QTI:`. Here we look at the templates `QTI: order (simple)` and `QTI: order (complex)`.
