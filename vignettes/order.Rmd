...
`r if(knitr::is_latex_output()) "# Order Exercises"`

In this type of exercise, the candidate has to bring a list of items in the correct order.
...
Knitting via the Knit-Button to qtijs, this exercise renders as`r ltx(" shown in Figure \\ref{order1qtijs}.", ":")`

![\label{order1qtijs}Simple order exercise rendered in qtijs](images/xml/order-simple.jpg){width=100%}

Alternatively, change the knit parameter to `knit: render_opal` (see article \ref{working-with-the-opal-api} [Working with the OPAL API](api_opal.html)) to upload to Opal directly, producing`r ltx(" the output in Figure \\ref{order1opal}.", ":")`

![\label{order1opal}Simple order exercise rendered in OPAL](images/order-simple.jpg){width=100%}
...
![\label{order2opal}More complex order exercise rendered in OPAL](images/order-complex.jpg){width=100%}
