...
In this type of exercise, the candidate has to bring a list of items in the correct order.A template is available in RStudio Project Template or can be added by clicking on `New file -> R Markdown -> From Template`.  The `rqti` templates start with `RQTI:`. Here we look at the templates `RQTI: order (simple)` and `RQTI: order (complex)`.
