...
test_that("test abbreviate for rmd2xml() in OneInRowTable class", {
    ...
    sut <- readLines(sut)
    unlink("to_delete", recursive = TRUE)
    expected <- readLines(test_path("file/xml/abbr_OneInRowTable.xml"))
...
test_that("test abbreviate for rmd2xml() in OneInColTable class", {
    ...
    sut <- readLines(sut)
    unlink("to_delete", recursive = TRUE)
    expected <- readLines(test_path("file/xml/abbr_OneInColTable.xml"))
    equal_xml(sut, expected)
})
...
test_that("test abbreviate for rmd2xml() in MultipleChoiceTable class", {
    ...
    sut <- readLines(sut)
    unlink("to_delete", recursive = TRUE)
    expected <- readLines(test_path("file/xml/abbr_MultipleChoiceTable.xml"))
    equal_xml(sut, expected)
})
...
test_that("test abbreviate for rmd2xml() in DirectPair class", {
    ...
    sut <- readLines(sut)
    unlink("to_delete", recursive = TRUE)
    expected <- readLines(test_path("file/xml/abbr_DirectedPair.xml"))
    equal_xml(sut, expected)
})
